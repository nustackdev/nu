...
# 1. Square root - a FloatForm (MathSqrt atom).
e1 = sqrt(2)
print(run(e1, ctx)[0], type(e1), e1)

# 2. Floor - returns an int, so a FloatForm in maps to an IntForm out.
e2 = floor(3.7)
print(run(e2, ctx)[0], type(e2), e2)

# 3. Greatest common divisor - two ints in, an IntForm out.
e3 = gcd(12, 8)
...
# 5. Euclidean norm of a 3-4-5 triangle - a FloatForm.
e5 = hypot(3, 4)
print(run(e5, ctx)[0], type(e5), e5)

# 6. NaN check - a BoolForm.
e6 = isnan(0.0)
