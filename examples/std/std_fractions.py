...
reads reuse core GetAttrQuery; method calls bind the unbound Fraction methods;
...
# 2. Exact rational arithmetic: 1/3 + 1/6 == 1/2 (core AddQuery).
e2 = (Fraction.of(1, 3) + Fraction.of(1, 6)).eq(Fraction.of(1, 2))
print(run(e2, ctx)[0], type(e2), e2)

# 3. Read the numerator (core GetAttrQuery), in lowest terms.
e3 = Fraction.of(2, 4).numerator()
print(run(e3, ctx)[0], type(e3), e3)

# 4. Read the denominator (core GetAttrQuery).
...
# 6. Comparison via the < operator (core LtQuery).
