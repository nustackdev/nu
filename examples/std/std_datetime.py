...
Property reads reuse core GetAttr; method calls reuse the shared
...
# 1. Build a date, read a component (GetAttr).
...
# 3. Date arithmetic: date + timedelta -> date (core Add), then isoformat.
e3 = (date.of(2026, 6, 30) + timedelta.of(days=5)).isoformat()
print(run(e3, ctx)[0], type(e3), e3)

# 4. Date difference: date - date -> timedelta (core Sub), total_seconds.
e4 = (date.of(2026, 7, 10) - date.of(2026, 6, 30)).total_seconds()
print(run(e4, ctx)[0], type(e4), e4)

# 5. timedelta scaling and a comparison (core Mul + Gt).
...
# 8. time comparison via the < operator (core Lt).
