...
# 1. Seconds since the epoch - a Float (TimeTime atom, non-deterministic).
e1 = time()
print(run(e1, ctx)[0], type(e1), e1)

# 2. A monotonic clock read - a Float (TimeMonotonic atom).
e2 = monotonic()
print(run(e2, ctx)[0], type(e2), e2)

# 3. The high-resolution timer - a Float (TimePerfCounter atom).
e3 = perf_counter()
print(run(e3, ctx)[0], type(e3), e3)

# 4. Nanoseconds since the epoch - an Int (TimeTimeNs atom).
e4 = time_ns()
print(run(e4, ctx)[0], type(e4), e4)

# 5. Block for 10ms - a None_ (TimeSleep Command; yields None, sync-only).
e5 = sleep(0.01)
