...
# 1. Build a complex number, take its magnitude - abs of 3+4j is 5.0 (FloatForm).
...
# 3. The real part - a property read (core GetAttrQuery).
e3 = complex.of(3, 4).real()
print(run(e3, ctx)[0], type(e3), e3)

# 4. The imaginary part - a property read (core GetAttrQuery).
...
# 6. The phase angle of 1j - pi/2, a FloatForm (CmathPhase atom).
e6 = phase(complex.of(0, 1))
print(run(e6, ctx)[0], type(e6), e6)

# 7. A constant - pi rides on LiteralQuery, no atom needed.
e7 = pi
