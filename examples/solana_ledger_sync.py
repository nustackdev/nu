...
Demonstrates: Shapes, Ref + method, Compositions (|, If, ForEach, Retry, TryCatch, Log),
Spans (nu_virtuals.Transaction), Deformations (inline_refs), Context (storage + service binding).
...
import aiohttp
import nu, nu_debugger, nu_dict, nu_virtuals
...
class SolanaRpc:
    ...
    def __init__(self, endpoint: str = "https://api.mainnet-beta.solana.com") -> None:
        self._endpoint, self._session, self._id = endpoint, None, 0
    ...
    async def get_blocks(self, start: int, end: int) -> list[int]:
        return [int(s) for s in await self._call("getBlocks", [start, end, {"commitment": "confirmed"}])]

    async def get_block(self, slot: int) -> list[dict]:
        """Fetch block, return tx dicts matching Transaction shape."""
        result = await self._call("getBlock", [slot, {
        ...
        for i, raw in enumerate(result.get("transactions", [])):
            ...
            programs = {accounts[ix.get("programIdIndex", 0)] for ix in all_ixs if ix.get("programIdIndex", 0) < len(accounts)}
            txs.append({
                "slot_number": slot, "block_time": block_time, "block_index": i,
...
class SolanaRef(nu.Ref[SolanaRpc]):
    ...
    async def resolve(self, ctx: nu.Context) -> str: return "solana_rpc"
    async def fetch(self, ctx: nu.Context) -> SolanaRpc: return ctx.get(SolanaRpc)
...
TX_ID_MULTIPLIER = 10_000  # tx_id = slot * 10_000 + block_index
...
def _persist_tx(ledger: type[Ledger], slot: nu.IntArg) -> nu.Nu:
    return (
        _SlotScratch.tx_id.store(nu.IntI(slot) * TX_ID_MULTIPLIER + nu.AtOp(nu.AttrRef("tx"), "block_index"))
    ...
    return nu.If(
        ...
        nu.Retry(
            nu.TryCatch(
                ...
                | nu_virtuals.Transaction(nu.ForEach(
                    ...
                    nu.If(
                        ...
                        _persist_tx(ledger, slot) | bm.synced.inc()
                        | nu.If((bm.synced % 10).eq(0), nu.Log("synced:", bm.synced)),
                    ),
                    item="tx",
                )),
                catch=nu_virtuals.Transaction(ledger.slots_dropped.add(slot)) | nu.Log("dropped slot", slot),
                errors=DroppedSlotError,
            ),
            max_attempts=5, delay=1, backoff=1.5,
...
def sync_range(ledger: type[Ledger], slot_from: int, slot_to: int, *, program_id: nu.StrArg = "") -> nu.Nu:
    """Sync all confirmed slots in [slot_from, slot_to]."""
    return (
        _RangeScratch.slots.store(SolanaRef.get_blocks(slot_from, slot_to))
        | nu.Log("sync:", slot_from, "->", slot_to, "(", nu.ops.Len(_RangeScratch.slots), "confirmed)")
        | nu.ForEach(_RangeScratch.slots, sync_slot(ledger, nu.IntAttrRef("slot"), program_id=program_id), item="slot")
...
async def main():
    ...
    async with SolanaRpc(endpoint=args.endpoint) as rpc:
        with rocksdb_storage_inmemory(args.db_path) as store:
            ctx = nu.Context().bind(Navigator, Navigator(store))
            ctx = ctx.bind(SolanaRpc, rpc).bind(dict, {})
