...
async def main() -> None:
    ...
    # single-yield value via `afirst`
    print(f"(3 + 4) * 2 = {await expr.afirst(ctx)}")

    # `acollect` gives a list (one element for a yield-once tree)
    print(f"collect: {await expr.acollect(ctx)}")

    # `aexecute` drains and returns None (algebra-faithful)
    result = await expr.aexecute(ctx)
    print(f"execute returns: {result}")

    # Sequential composition with `>>`
    seq = nu.Literal(1) >> nu.Literal(2) >> nu.Literal(3)
    print(f"1 >> 2 >> 3 collect: {await seq.acollect(ctx)}")

    # Parallel composition with `|` (interleaves)
    par = nu.Literal("a") | nu.Literal("b") | nu.Literal("c")
    print(f"a | b | c collect: {sorted(await par.acollect(ctx))}")
    ...
    print(f"AttrGet via first: {await greeting_ref.afirst(ctx2)}")
    # AttrExistsOp is a pure Query too
    from nu.context.attr_ops import AttrExistsOp
    print(f"'greeting' exists: {await AttrExistsOp(greeting_ref).afirst(ctx2)}")
    print(f"'missing' exists: {await AttrExistsOp(nu.StrAttrRef('missing')).afirst(ctx2)}")
