...
Each tree wraps its per-worker ``Teleport`` calls in an outer ``Add`` -
...
async def demo_fleet_by_index() -> None:
    ...
    tree = Provide(
        ...
        ProvideList(
            ...
            Add(
                Teleport(Add(1, 2), target=0),  # 3
                Teleport(Add(10, 20), target=1),  # 30
                Teleport(Add(100, 200), target=2),  # 300
            ),
        ),
    )
    result, _ = await arun(tree)
...
async def demo_keyed_fleet() -> None:
    ...
    tree = Provide(
        ...
        ProvideDict(
            ...
            Add(
                Teleport(Add(1, 2), target=("ledger", 0)),  # 3
                Teleport(Add(3, 4), target=("ledger", 1)),  # 7
                Teleport(Add(5, 6), target="ledger-main"),  # 11
            ),
        ),
    )
    result, _ = await arun(tree)
