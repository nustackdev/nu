...
class TestDecimalSetAndGet:
    ...
    async def test_set_and_get_decimal(self, decimal_shape, ctx):
        ...
        d = Decimal("1234.56")
        await decimal_shape.balance.set(d).execute(ctx)
        result = await decimal_shape.balance.get().execute(ctx)
        assert result == d

    async def test_set_decimal_from_string(self, decimal_shape, ctx):
        """Set Decimal from string."""
        await decimal_shape.balance.set("999.99").execute(ctx)
        result = await decimal_shape.balance.get().execute(ctx)
    ...
    async def test_set_multiple_decimals(self, decimal_shape, ctx):
        ...
        await decimal_shape.balance.set(balance).execute(ctx)
        await decimal_shape.credit.set(credit).execute(ctx)

        assert await decimal_shape.balance.get().execute(ctx) == balance
        assert await decimal_shape.credit.get().execute(ctx) == credit
...
class TestDecimalArithmetic:
    ...
    async def test_addition(self, decimal_shape, ctx):
        """Add two Decimals."""
        await decimal_shape.balance.set(Decimal("100.00")).execute(ctx)
        result = await (decimal_shape.balance.get() + Decimal("50.25")).execute(ctx)
        assert result == Decimal("150.25")

    async def test_subtraction(self, decimal_shape, ctx):
        """Subtract Decimals."""
        await decimal_shape.balance.set(Decimal("100.00")).execute(ctx)
        result = await (decimal_shape.balance.get() - Decimal("25.50")).execute(ctx)
        assert result == Decimal("74.50")

    async def test_multiplication(self, decimal_shape, ctx):
        """Multiply Decimals."""
        await decimal_shape.balance.set(Decimal("100.00")).execute(ctx)
        result = await (decimal_shape.balance.get() * Decimal("1.05")).execute(ctx)
        assert result == Decimal("105.0000")

    async def test_division(self, decimal_shape, ctx):
        """Divide Decimals."""
        await decimal_shape.balance.set(Decimal("100.00")).execute(ctx)
        result = await (decimal_shape.balance.get() / Decimal("4")).execute(ctx)
        assert result == Decimal("25")

    async def test_floor_division(self, decimal_shape, ctx):
        """Floor divide Decimals."""
        await decimal_shape.balance.set(Decimal("100.00")).execute(ctx)
        result = await (decimal_shape.balance.get() // Decimal("3")).execute(ctx)
        assert result == Decimal("33")

    async def test_modulo(self, decimal_shape, ctx):
        """Modulo operation."""
        await decimal_shape.balance.set(Decimal("100.00")).execute(ctx)
        result = await (decimal_shape.balance.get() % Decimal("30")).execute(ctx)
        assert result == Decimal("10.00")

    async def test_power(self, decimal_shape, ctx):
        """Power operation."""
        await decimal_shape.balance.set(Decimal("2")).execute(ctx)
        result = await (decimal_shape.balance.get() ** 10).execute(ctx)
...
class TestDecimalRounding:
    ...
    async def test_quantize(self, decimal_shape, ctx):
        ...
        await decimal_shape.balance.set(Decimal("123.456789")).execute(ctx)
        result = await decimal_shape.balance.get().quantize(Decimal("0.01")).execute(ctx)
        assert result == Decimal("123.46")

    async def test_normalize(self, decimal_shape, ctx):
        """Remove trailing zeros."""
        await decimal_shape.balance.set(Decimal("123.4500")).execute(ctx)
        result = await decimal_shape.balance.get().normalize().execute(ctx)
...
class TestDecimalMathFunctions:
    ...
    async def test_sqrt(self, decimal_shape, ctx):
        """Square root."""
        await decimal_shape.balance.set(Decimal("16")).execute(ctx)
        result = await decimal_shape.balance.get().sqrt().execute(ctx)
    ...
    async def test_ln(self, decimal_shape, ctx):
        ...
        await decimal_shape.balance.set(Decimal("1")).execute(ctx)
        result = await decimal_shape.balance.get().exp().execute(ctx)
        assert abs(float(result) - e) < 0.0001

    async def test_log10(self, decimal_shape, ctx):
        """Base-10 logarithm."""
        await decimal_shape.balance.set(Decimal("100")).execute(ctx)
        result = await decimal_shape.balance.get().log10().execute(ctx)
...
class TestDecimalInspection:
    ...
    async def test_is_finite(self, decimal_shape, ctx):
        ...
        await decimal_shape.balance.set(Decimal("123.45")).execute(ctx)
        result = await decimal_shape.balance.get().is_finite().execute(ctx)
        assert result is True

    async def test_is_zero(self, decimal_shape, ctx):
        """Check if value is zero."""
        await decimal_shape.balance.set(Decimal("0")).execute(ctx)
        result = await decimal_shape.balance.get().is_zero().execute(ctx)
        assert result is True

    async def test_is_signed(self, decimal_shape, ctx):
        """Check if value is signed (negative)."""
        await decimal_shape.balance.set(Decimal("-123.45")).execute(ctx)
        result = await decimal_shape.balance.get().is_signed().execute(ctx)
        assert result is True
...
class TestDecimalConversions:
    ...
    async def test_to_int(self, decimal_shape, ctx):
        """Convert to integer."""
        await decimal_shape.balance.set(Decimal("123.99")).execute(ctx)
        result = await decimal_shape.balance.get().to_int().execute(ctx)
        assert result == 123
