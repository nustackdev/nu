...
class TestUUIDSetAndGet:
    ...
    async def test_set_and_get_uuid(self, uuid_shape, ctx):
        ...
        u = uuid4()
        await uuid_shape.id.set(u).execute(ctx)
        result = await uuid_shape.id.get().execute(ctx)
        assert result == u

    async def test_set_uuid_from_string(self, uuid_shape, ctx):
        """Set UUID from string."""
        uuid_str = "12345678-1234-5678-1234-567812345678"
        await uuid_shape.id.set(uuid_str).execute(ctx)
        result = await uuid_shape.id.get().execute(ctx)
    ...
    async def test_set_multiple_uuids(self, uuid_shape, ctx):
        ...
        await uuid_shape.id.set(id1).execute(ctx)
        await uuid_shape.parent_id.set(id2).execute(ctx)

        assert await uuid_shape.id.get().execute(ctx) == id1
        assert await uuid_shape.parent_id.get().execute(ctx) == id2
...
class TestUUIDRefConstructors:
    ...
    async def test_from_str(self, ctx):
        """Create from string."""
        uuid_str = "12345678-1234-5678-1234-567812345678"
        result = await UUIDRef.from_str(uuid_str).execute(ctx)
        assert result == UUID(uuid_str)
    ...
    async def test_from_bytes(self, ctx):
        ...
        result = await UUIDRef.from_bytes(uuid_bytes).execute(ctx)
    ...
    async def test_from_int(self, ctx):
        ...
        result = await UUIDRef.from_int(uuid_int).execute(ctx)
...
class TestUUIDComponentAccess:
    ...
    async def test_version(self, uuid_shape, ctx):
        ...
        await uuid_shape.id.set(u).execute(ctx)
        result = await uuid_shape.id.get().version().execute(ctx)
        assert result == 4
    ...
    async def test_variant(self, uuid_shape, ctx):
        ...
        u = await UUIDRef.uuid4().execute(ctx)
        await uuid_shape.id.set(u).execute(ctx)
        result = await uuid_shape.id.get().variant().execute(ctx)
    ...
    async def test_time(self, uuid_shape, ctx):
        ...
        await uuid_shape.id.set(u).execute(ctx)
        result = await uuid_shape.id.get().time().execute(ctx)
    ...
    async def test_clock_seq(self, uuid_shape, ctx):
        ...
        u = await UUIDRef.uuid1().execute(ctx)
        await uuid_shape.id.set(u).execute(ctx)
        result = await uuid_shape.id.get().clock_seq().execute(ctx)
        assert isinstance(result, int)
    ...
    async def test_node(self, uuid_shape, ctx):
        ...
        u = await UUIDRef.uuid1().execute(ctx)
        await uuid_shape.id.set(u).execute(ctx)
        result = await uuid_shape.id.get().node().execute(ctx)
        assert isinstance(result, int)
...
class TestUUIDConversions:
    ...
    async def test_hex(self, uuid_shape, ctx):
        ...
        uuid_str = "12345678-1234-5678-1234-567812345678"
        await uuid_shape.id.set(UUID(uuid_str)).execute(ctx)
        result = await uuid_shape.id.get().hex().execute(ctx)
    ...
    async def test_urn(self, uuid_shape, ctx):
        ...
        uuid_str = "12345678-1234-5678-1234-567812345678"
        await uuid_shape.id.set(UUID(uuid_str)).execute(ctx)
        result = await uuid_shape.id.get().urn().execute(ctx)
    ...
    async def test_bytes(self, uuid_shape, ctx):
        ...
        u = uuid4()
        await uuid_shape.id.set(u).execute(ctx)
        result = await uuid_shape.id.get().bytes().execute(ctx)
    ...
    async def test_bytes_le(self, uuid_shape, ctx):
        ...
        u = uuid4()
        await uuid_shape.id.set(u).execute(ctx)
        result = await uuid_shape.id.get().bytes_le().execute(ctx)
        assert len(result) == 16
    ...
    async def test_int(self, uuid_shape, ctx):
        ...
        uuid_str = "12345678-1234-5678-1234-567812345678"
        await uuid_shape.id.set(UUID(uuid_str)).execute(ctx)
        result = await uuid_shape.id.get().int_().execute(ctx)
...
class TestUUIDEquality:
    ...
    async def test_equals(self, uuid_shape, ctx):
        ...
        u = uuid4()
        await uuid_shape.id.set(u).execute(ctx)
        await uuid_shape.parent_id.set(u).execute(ctx)

        result = await uuid_shape.id.get().eq(uuid_shape.parent_id.get()).execute(ctx)
        assert result is True

    async def test_not_equals(self, uuid_shape, ctx):
        """Compare UUIDs for inequality."""
        await uuid_shape.id.set(uuid4()).execute(ctx)
        await uuid_shape.parent_id.set(uuid4()).execute(ctx)

        result = await uuid_shape.id.get().ne(uuid_shape.parent_id.get()).execute(ctx)
        assert result is True
...
class TestUUIDDeterministic:
    ...
    async def test_uuid3_same_input_same_output(self, ctx):
        ...
        result1 = await UUIDRef.uuid3(NAMESPACE_DNS, "example.com").execute(ctx)
        result2 = await UUIDRef.uuid3(NAMESPACE_DNS, "example.com").execute(ctx)
        assert result1 == result2

    async def test_uuid3_different_input_different_output(self, ctx):
        """UUID3 with different input produces different output."""
        result1 = await UUIDRef.uuid3(NAMESPACE_DNS, "example.com").execute(ctx)
        result2 = await UUIDRef.uuid3(NAMESPACE_DNS, "other.com").execute(ctx)
        assert result1 != result2

    async def test_uuid5_same_input_same_output(self, ctx):
        """UUID5 with same input produces same output."""
        result1 = await UUIDRef.uuid5(NAMESPACE_DNS, "example.com").execute(ctx)
        result2 = await UUIDRef.uuid5(NAMESPACE_DNS, "example.com").execute(ctx)
        assert result1 == result2

    async def test_uuid5_different_input_different_output(self, ctx):
        """UUID5 with different input produces different output."""
        result1 = await UUIDRef.uuid5(NAMESPACE_DNS, "example.com").execute(ctx)
        result2 = await UUIDRef.uuid5(NAMESPACE_DNS, "other.com").execute(ctx)
        assert result1 != result2
