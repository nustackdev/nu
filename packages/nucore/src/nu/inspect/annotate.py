...
``nu.inspect`` is kernel, ``nu.std`` is batteries (the ``nustd``
distribution), so the ``Log`` atom is imported lazily inside
``annotate_retries`` rather than at module import. ``annotate_steps`` needs
no atom at all - it logs imperatively - so it works on a bare kernel install.
"""
...
# Mirrors ``nu.std.logging.interactions._LEVELS``. Duplicated (six entries) so
# the kernel does not reach into the batteries package for a level lookup.
...
def _emit_log(level: str, logger: str, message: str) -> None:
    ...
    """
    code = level if isinstance(level, int) else _LEVELS.get(str(level).lower(), _pylogging.INFO)
    _pylogging.getLogger(logger).log(code, message)
...
def annotate_retries(tree: Nu, *, logger: str = _RETRY_LOGGER) -> Nu:
    ...
    """
    # ``Log`` is a nu.std atom (batteries); import it here so a kernel-only
    # install can still import nu.inspect and use annotate_steps.
    try:
        from nu.std.logging import Log
    except ImportError as exc:  # pragma: no cover - only on a kernel-only install
        msg = "annotate_retries needs the nu.std fabric: pip install nustd"
