"""Factory for creating EveryFlow resources remotely via RPC."""
...
if TYPE_CHECKING:
    from link import Spec, SyncResource
...
class ResourceFactory:
    ...
    def exposed_get_resource(self, spec_data: bytes) -> object:
        ...
        Raises:
            InvisiblesServerError: If resource creation fails
        """
        ...
        try:
            ...
            from link import ProxySpec, ResourceSpec

            curr_spec = spec
            while True:
                if isinstance(curr_spec, ResourceSpec):
                    cls = curr_spec.factory
                    break
                elif isinstance(curr_spec, ProxySpec):
                    curr_spec = curr_spec.inner_spec

            # Create the resource using its factory
            resource = cast("SyncResource", cls(spec))
            ...
            raise InvisiblesServerError(f"Failed to create resource {spec}") from e
    ...
    def exposed_remove_resource(self, spec_data: bytes) -> bool:
        ...
        Args:
            spec_data: Specification of resource to remove
    ...
    def _deserialize_spec(serialized_spec: bytes) -> Spec:
        ...
        return pickle.loads(serialized_spec)  # noqa: S301
