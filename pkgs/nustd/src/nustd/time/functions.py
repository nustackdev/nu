"""Module-level functions for ``nustd.time`` - the function namespace.
...
builds its interaction atom (lazily imported, like ``nustd.math``) and returns
...
effect-only - see ``interactions``. The async sleep lives in ``nustd.asyncio``.
