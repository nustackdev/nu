...
from .._types import ContainerProtocol, ContainerStructure
...
class BaseView(TransactionalBase, ABC):
    ...
    path: StatePath = attrs.field(eq=False, hash=False, alias=None)
    ...
    def root(self, *, tx: Optional[ObservableKVTransaction] = None) -> "State":
        ...
        Example:
            ```python
            # Navigate to root from any view
            deep_dict = state.at("deeply", "nested", "path").dict_view()
            root_state = deep_dict.root()  # State for root

            # Or from within a context manager
            with state.at("users").with_dict_view() as users:
                root_state = users.root()
                root_dict = root_state.dict_view()  # Inherits transaction
            ```
        """
        # Import here to avoid circular imports
        from .._state.state import State

        return State(backend=self.backend, path=StatePath(), tx=tx or self.tx)
