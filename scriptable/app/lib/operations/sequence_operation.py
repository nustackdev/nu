...
class SequenceOperation(Operation):
    ...
    def _initialize_state(self, service: "Service") -> None:
    ...
    def _update_step(self, service: "Service", step: int) -> None:
        """Update current execution step in store"""
        service.state_set(self._state_key("current_step"), value=step)
        service.state_set(self._state_key("status"), value="running")

    def _record_error(self, service: "Service", step: int, error: Exception) -> None:
        """Record operation error in store"""
        error_data = {"step": step, "error": str(error), "error_type": error.__class__.__name__}
        service.state_set(self._state_key("errors"), value=lambda errors: [*errors, error_data])
    ...
    def execute(self, service: "Service") -> None:
        ...
        try:
            self._initialize_state(service)

            for i, operation in enumerate(self.operations):
                try:
                    self._update_step(service, i)
                    ...
                    self._record_error(service, i, e)

                    if not self.continue_on_error:
                        raise OperationError(error_msg) from e

            service.state_set(self._state_key("status"), value="completed")
            logger.info("Sequence execution completed successfully")

        except Exception as e:
            service.state_set(self._state_key("status"), value="failed")
