...
class Address(nu.Shape):
    street: StrRef
    city: StrRef
    zipcode: ZipCode = nustd.kv.ShapeRef.slot(ZipCode)


class Profile(nu.Shape):
    name: StrRef
    age: IntRef
    address: Address = nustd.kv.ShapeRef.slot(Address)


class User(nu.Shape):
    handle: StrRef
    profile: Profile = nustd.kv.ShapeRef.slot(Profile)
...
direct = nustd.kv.ShapeRef.slot(Profile)
