...
async def test_add_commutative(a, b):
    ctx = Context()
    r1 = await Add(a, b).first(ctx)
    r2 = await Add(b, a).first(ctx)
    assert r1 == r2
...
async def test_mul_commutative(a, b):
    ctx = Context()
    r1 = await Mul(a, b).first(ctx)
    r2 = await Mul(b, a).first(ctx)
    assert r1 == r2
...
async def test_add_identity(a):
    ctx = Context()
    assert await Add(a, 0).first(ctx) == a


@given(a=numbers)
async def test_mul_identity(a):
    ctx = Context()
    assert await Mul(a, 1).first(ctx) == a
...
async def test_neg_involution(a):
    ...
    ctx = Context()
    result = await Neg(Neg(a)).first(ctx)
    assert result == a
...
async def test_sub_self_is_zero(a):
    ctx = Context()
    assert await Sub(a, a).first(ctx) == 0
...
async def test_div_mul_inverse(a, b):
    ...
    ctx = Context()
    q = await FloorDiv(a, b).first(ctx)
    r = await Mod(a, b).first(ctx)
    assert q * b + r == a
...
async def test_pow_matches_python(a, b):
    ctx = Context()
    assert await Pow(a, b).first(ctx) == a**b
...
async def test_add_strings(ctx):
    ...
    assert await Add("hello ", "world").first(ctx) == "hello world"


async def test_mul_string_repeat(ctx):
    """String repetition via Mul."""
    assert await Mul("ab", 3).first(ctx) == "ababab"


async def test_div_returns_float(ctx):
    assert await Div(7, 2).first(ctx) == 3.5


async def test_floor_div_truncates(ctx):
    assert await FloorDiv(7, 2).first(ctx) == 3


async def test_floor_div_negative(ctx):
    assert await FloorDiv(-7, 2).first(ctx) == -4
