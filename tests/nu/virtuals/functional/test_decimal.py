...
class TestDecimalSetAndGet:
    ...
    async def test_set_and_get_decimal(self, decimal_shape, ctx):
        ...
        d = Decimal("1234.56")
        await decimal_shape.balance.store(d).aexecute(ctx)
        result = await decimal_shape.balance.aexecute(ctx)
        assert result == d

    async def test_set_decimal_from_string(self, decimal_shape, ctx):
        """Set Decimal from string."""
        await decimal_shape.balance.store("999.99").aexecute(ctx)
        result = await decimal_shape.balance.aexecute(ctx)
    ...
    async def test_set_multiple_decimals(self, decimal_shape, ctx):
        ...
        await decimal_shape.balance.store(balance).aexecute(ctx)
        await decimal_shape.credit.store(credit).aexecute(ctx)

        assert await decimal_shape.balance.aexecute(ctx) == balance
        assert await decimal_shape.credit.aexecute(ctx) == credit
...
class TestDecimalRefConstructors:
    ...
    async def test_from_str(self, ctx):
        """Create from string."""
        result = await DecimalRef.from_str("123.456").aexecute(ctx)
        assert result == Decimal("123.456")

    async def test_from_int(self, ctx):
        """Create from integer."""
        result = await DecimalRef.from_int(42).aexecute(ctx)
        assert result == Decimal(42)

    async def test_from_float(self, ctx):
        """Create from float (with precision caveat)."""
        result = await DecimalRef.from_float(3.14).aexecute(ctx)
...
class TestDecimalArithmetic:
    ...
    async def test_addition(self, decimal_shape, ctx):
        """Add two Decimals."""
        await decimal_shape.balance.store(Decimal("100.00")).aexecute(ctx)
        result = await (decimal_shape.balance + Decimal("50.25")).aexecute(ctx)
        assert result == Decimal("150.25")

    async def test_subtraction(self, decimal_shape, ctx):
        """Subtract Decimals."""
        await decimal_shape.balance.store(Decimal("100.00")).aexecute(ctx)
        result = await (decimal_shape.balance - Decimal("25.50")).aexecute(ctx)
        assert result == Decimal("74.50")

    async def test_multiplication(self, decimal_shape, ctx):
        """Multiply Decimals."""
        await decimal_shape.balance.store(Decimal("100.00")).aexecute(ctx)
        result = await (decimal_shape.balance * Decimal("1.05")).aexecute(ctx)
        assert result == Decimal("105.0000")

    async def test_division(self, decimal_shape, ctx):
        """Divide Decimals."""
        await decimal_shape.balance.store(Decimal("100.00")).aexecute(ctx)
        result = await (decimal_shape.balance / Decimal("4")).aexecute(ctx)
        assert result == Decimal("25")

    async def test_floor_division(self, decimal_shape, ctx):
        """Floor divide Decimals."""
        await decimal_shape.balance.store(Decimal("100.00")).aexecute(ctx)
        result = await (decimal_shape.balance // Decimal("3")).aexecute(ctx)
        assert result == Decimal("33")

    async def test_modulo(self, decimal_shape, ctx):
        """Modulo operation."""
        await decimal_shape.balance.store(Decimal("100.00")).aexecute(ctx)
        result = await (decimal_shape.balance % Decimal("30")).aexecute(ctx)
        assert result == Decimal("10.00")

    async def test_power(self, decimal_shape, ctx):
        """Power operation."""
        await decimal_shape.balance.store(Decimal("2")).aexecute(ctx)
        result = await (decimal_shape.balance**10).aexecute(ctx)
...
class TestDecimalRounding:
    ...
    async def test_quantize(self, decimal_shape, ctx):
        ...
        await decimal_shape.balance.store(Decimal("123.456789")).aexecute(ctx)
        result = await decimal_shape.balance.quantize(Decimal("0.01")).aexecute(ctx)
        assert result == Decimal("123.46")

    async def test_normalize(self, decimal_shape, ctx):
        """Remove trailing zeros."""
        await decimal_shape.balance.store(Decimal("123.4500")).aexecute(ctx)
        result = await decimal_shape.balance.normalize().aexecute(ctx)
...
class TestDecimalMathFunctions:
    ...
    async def test_sqrt(self, decimal_shape, ctx):
        """Square root."""
        await decimal_shape.balance.store(Decimal("16")).aexecute(ctx)
        result = await decimal_shape.balance.sqrt().aexecute(ctx)
    ...
    async def test_ln(self, decimal_shape, ctx):
        ...
        await decimal_shape.balance.store(Decimal("1")).aexecute(ctx)
        result = await decimal_shape.balance.exp().aexecute(ctx)
        assert abs(float(result) - e) < 0.0001

    async def test_log10(self, decimal_shape, ctx):
        """Base-10 logarithm."""
        await decimal_shape.balance.store(Decimal("100")).aexecute(ctx)
        result = await decimal_shape.balance.log10().aexecute(ctx)
...
class TestDecimalInspection:
    ...
    async def test_is_finite(self, decimal_shape, ctx):
        ...
        await decimal_shape.balance.store(Decimal("123.45")).aexecute(ctx)
        result = await decimal_shape.balance.is_finite().aexecute(ctx)
        assert result is True

    async def test_is_zero(self, decimal_shape, ctx):
        """Check if value is zero."""
        await decimal_shape.balance.store(Decimal("0")).aexecute(ctx)
        result = await decimal_shape.balance.is_zero().aexecute(ctx)
        assert result is True

    async def test_is_signed(self, decimal_shape, ctx):
        """Check if value is signed (negative)."""
        await decimal_shape.balance.store(Decimal("-123.45")).aexecute(ctx)
        result = await decimal_shape.balance.is_signed().aexecute(ctx)
        assert result is True
...
class TestDecimalConversions:
    ...
    async def test_to_int(self, decimal_shape, ctx):
        """Convert to integer."""
        await decimal_shape.balance.store(Decimal("123.99")).aexecute(ctx)
        result = await decimal_shape.balance.to_int().aexecute(ctx)
        assert result == 123
