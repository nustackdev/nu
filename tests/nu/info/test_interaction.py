"""Tests for nu.info.interaction - the guide, the checker, the assembler.

The load-bearing test here is ``test_the_guides_own_example_conforms``: the
guide splices in the source of ``Clamp``, and the validator runs on the same
class, so the contract cannot drift from the checker.
"""
...
from nu.info import GUIDE, catalogue, parse_interaction, validate_interaction
from nu.info.interaction import unpacked_arity
from nu.info.interaction.guide import Clamp


# --- guide ---------------------------------------------------------------


def test_the_guides_own_example_conforms() -> None:
    assert validate_interaction(Clamp) == []


def test_the_guide_carries_that_example_verbatim() -> None:
    assert "class Clamp(ScalarQuery):" in GUIDE
    assert "The first child, held between the second and the third." in GUIDE


def test_the_guide_names_every_section_of_the_contract() -> None:
    for section in ("Summary", "Description", "Args", "Notes", "Yields", "Example"):
...
def test_missing_docstring_reports_one_problem() -> None:
    assert [p.rule for p in validate_interaction(_NoDoc)] == ["summary-missing"]


def test_a_summary_alone_is_missing_the_three_required_sections() -> None:
    assert [p.rule for p in validate_interaction(_SummaryOnly)] == [
        "args-missing",
        "yields-missing",
        "example-missing",
    ]
...
def test_unterminated_summary_is_reported_with_the_text() -> None:
    (problem,) = validate_interaction(_Unterminated)
    assert problem.rule == "summary-unterminated"
    assert problem.detail == "A summary with no full stop"


def test_documented_arity_is_checked_against_what_compile_unpacks() -> None:
    (problem,) = validate_interaction(_Mismatch)
    assert problem.rule == "args-arity-mismatch"
    assert problem.detail == "documents 3, code takes 2"


def test_a_command_is_not_asked_what_it_yields() -> None:
    rules = {p.rule for p in validate_interaction(core.Print)}
    assert "yields-missing" not in rules


def test_validation_never_raises_on_a_real_atom() -> None:
    for record in catalogue(core):
        assert isinstance(validate_interaction(record.target), list)
...
def test_record_carries_summary_notes_and_yields_apart() -> None:
    record = parse_interaction(Clamp)
    assert record.summary == "The first child, held between the second and the third."
    assert record.description == ""  # a discrete fact is a note, not a description
    assert record.notes == (
        "A value already inside the bounds is yielded unchanged.",
        "The bounds are ordinary children, so either can be a Ref read at runtime rather "
        "than a constant.",
        "Bounds the wrong way round yield INVALID rather than swapping.",
    )
    assert record.yields.startswith("The held value.")
...
def test_record_example_keeps_the_value_it_produces() -> None:
    example = parse_interaction(Clamp).example
    assert example.doctest
    assert example.code == "nu.run(Clamp(nu.Int(12), nu.Int(0), nu.Int(10)))[0]"
    assert example.expected == "10"
...
def test_args_come_from_the_docstring_when_the_constructor_is_inherited() -> None:
    record = parse_interaction(Clamp)
    assert [arg.name for arg in record.args] == ["value", "low", "high"]
    assert record.arity == 3
