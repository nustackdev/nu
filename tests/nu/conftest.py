...
Layout: mirrors ``src/nu/`` for unit tests, with a top-level ``integration/``
...
``from _support.terms import Leaf`` from any test file under ``tests/nu/``.
The leading underscore keeps pytest from auto-collecting it as tests; the
``pythonpath = ["tests/nu"]`` entry in ``pyproject.toml`` makes it
importable.
...
- **No ``__init__.py`` under ``tests/nu/`` (except in ``_support/``).** With
