"""Tests for nu.inspect.interaction: parser and verifier.
...
from nu.inspect.interaction import unpacked_arity
...
def test_catalogue_covers_every_exported_interaction() -> None:
    ...
    assert {"Add", "Map", "Sum", "Print"} <= names
    assert all(r.path.startswith("nu.core.") for r in catalogue(core))
    assert {"Sequential", "IfDo", "Race"} <= {r.name for r in catalogue(flows)}
    # Literal is a language primitive, not a core atom: it lives in nu.lang.
    assert "Literal" in {r.name for r in catalogue(lang)}
