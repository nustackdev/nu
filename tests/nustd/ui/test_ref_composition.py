...
from nustd.ui.refs import Row, TextRef
...
def test_kv_slot_on_a_page_stays_a_kv_path():
    ...
    ref = Mixed.count
    assert isinstance(ref, nustd.kv.IntRef)
...
def test_input_read_resolves_path_and_reads_session():
    ...
    from nustd.ui.refs import InputRef
...
def _leaf_widgets(*, needs_on_change: bool = False):
    ...
    root = Path(importlib.import_module("nustd.ui.refs").__file__).parent
    out = []
    for entry in sorted(root.iterdir()):
        fname = entry.name
        if not fname.endswith(".py") or fname.startswith("_"):
            continue
        mod = importlib.import_module("nustd.ui.refs." + fname[:-3])
...
def _shipped_ref_classes():
    """Every Ref and Section class the kit defines under nustd.ui.refs."""
    import importlib
    import inspect
    from pathlib import Path

    from nustd.ui.core import Ref, Section
...
def test_tabs_set_active_emits_frame():
    ...
    from nustd.ui.refs import Tabs
...
def test_card_set_title_emits_frame():
    from nustd.ui.refs import Card
