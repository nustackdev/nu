"""Functional tests for nustd.http: HttpFabric + 5 verb interactions + bind.
...
by monkey-patching `httpx.AsyncClient` inside `nustd.http.fabric`, so the real
...
def _patch_httpx_client(monkeypatch, calls):
    ...
    monkeypatch.setattr("nustd.http.fabric.httpx.AsyncClient", _Client)
...
class GH(nu.Service):
    ...
    echo_get = nustd.http.GETRef.method("/echo")
    echo_post = nustd.http.POSTRef.method("/echo")
    echo_put = nustd.http.PUTRef.method("/echo")
    echo_patch = nustd.http.PATCHRef.method("/echo")
...
def app():
    def make(body):
        return nu.With(
            nustd.http.bind(GH, base_url="https://api.example.com"),
            body=body,
        )
...
async def test_declaration_defaults_flow_into_body(app, calls):
    class Svc(nu.Service):
        echo = nustd.http.POSTRef.method("/echo", version=1, source="test")

    tree = nu.With(
        nustd.http.bind(Svc, base_url="https://api.example.com"),
...
async def test_non_2xx_raises_http_status_error(app):
    class Svc(nu.Service):
        missing = nustd.http.GETRef.method("/nope")
...
async def test_bind_tags_by_service_class(calls):
    class SvcA(nu.Service):
        one = nustd.http.GETRef.method("/repos/nu/core")

    class SvcB(nu.Service):
        one = nustd.http.GETRef.method("/echo")

    tree = nu.With(
        nustd.http.bind(SvcA, base_url="https://a.example.com"),
        nustd.http.bind(SvcB, base_url="https://b.example.com"),
        body=SvcA.one(),
    )
    value, _ = await nu.arun(tree)
    assert value == {"name": "core", "stars": 42}
