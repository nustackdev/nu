"""Functional tests for Nu rebuilt on the attribute layer (nu.lang).
...
def test_ref_in_a_query_slot_tracks_a_read():
    program = compile(ScalarQuery(Ref("counter")))
    assert program.attr(program.root, Attr.COMPOSITION_EFFECTS) == frozenset(
        {("counter", Effect.READ)}
    )
...
def test_effects_fold_up_the_subtree():
    program = compile(Write(Ref("total"), ScalarQuery(Ref("total"))))
    assert program.attr(program.root, Attr.COMPOSITION_EFFECTS) == frozenset(
...
def test_cardinality_is_fixed_by_kind():
    program = compile(ScalarQuery(StreamQuery()))
...
def test_has_async_only_atom_only_when_an_async_atom_is_present():
    assert compile(ScalarQuery()).attr((), Attr.HAS_ASYNC_ONLY_ATOM) is False
    assert compile(AsyncWrite(Ref("u"))).attr((), Attr.HAS_ASYNC_ONLY_ATOM) is True


def test_on_loop_threads_from_the_root():
    assert compile(ScalarQuery()).attr((), Attr.ON_LOOP) is False
    assert compile(AsyncWrite(Ref("u"))).attr((), Attr.ON_LOOP) is True


def test_parallel_parent_resolves_on_loop_per_child():
    program = compile(Fork(AsyncWrite(Ref("u")), SyncWrite(Ref("d"))))
...
def test_composition_matrix_rejects_a_command_in_a_query():
    verdict = gate(compile(ScalarQuery(Write(Ref("x")))), *LAWS)
...
def test_sync_atom_on_the_loop_is_a_warning_not_a_rejection():
    ...
    program = compile(Strategy(AsyncWrite(Ref("u")), SyncWrite(Ref("d"))))
...
def test_validate_passes_a_clean_program_and_raises_on_a_bad_one():
    clean = compile(Strategy(Write(Ref("a")), Write(Ref("b"))))
    assert validate(clean, *LAWS) is clean

    with pytest.raises(ValueError, match="ref_slots"):
        validate(compile(Write(ScalarQuery())), *LAWS)


def test_attr_is_queryable_as_a_relation():
    program = compile(Write(Ref("x"), ScalarQuery(Ref("x"))))
