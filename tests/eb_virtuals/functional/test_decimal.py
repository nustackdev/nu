...
# ============================================================================
# DECIMAL STORE AND LOAD TESTS
# ============================================================================


class TestDecimalStoreAndLoad:
    """Test storing and loading Decimal values through storage."""

    async def test_store_and_load_decimal(self, decimal_shape, ctx):
        """Store and retrieve a Decimal value."""
        d = Decimal("1234.56")
        await decimal_shape.balance.store(d).execute(ctx)
        result = await decimal_shape.balance.load().execute(ctx)
        assert result == d

    async def test_store_decimal_from_string(self, decimal_shape, ctx):
        """Store Decimal from string."""
        await decimal_shape.balance.store("999.99").execute(ctx)
        result = await decimal_shape.balance.load().execute(ctx)
        assert result == Decimal("999.99")

    async def test_store_multiple_decimals(self, decimal_shape, ctx):
        """Store multiple Decimal slots."""
        ...
        assert await decimal_shape.balance.load().execute(ctx) == balance
        assert await decimal_shape.credit.load().execute(ctx) == credit
...
class TestDecimalRounding:
    ...
    async def test_quantize(self, decimal_shape, ctx):
        ...
        result = await decimal_shape.balance.load().quantize(Decimal("0.01")).execute(ctx)
        assert result == Decimal("123.46")

    async def test_normalize(self, decimal_shape, ctx):
        """Remove trailing zeros."""
        await decimal_shape.balance.store(Decimal("123.4500")).execute(ctx)
        result = await decimal_shape.balance.load().normalize().execute(ctx)
...
class TestDecimalMathFunctions:
    ...
    async def test_sqrt(self, decimal_shape, ctx):
        ...
        result = await decimal_shape.balance.load().sqrt().execute(ctx)
    ...
    async def test_ln(self, decimal_shape, ctx):
        ...
        result = await decimal_shape.balance.load().exp().execute(ctx)
    ...
    async def test_log10(self, decimal_shape, ctx):
        ...
        result = await decimal_shape.balance.load().log10().execute(ctx)
...
class TestDecimalConversions:
    ...
    async def test_to_int(self, decimal_shape, ctx):
        ...
        result = await decimal_shape.balance.load().to_int().execute(ctx)
        assert result == 123
