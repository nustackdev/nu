"""Unit tests for Percentage type.

Tests for:
- Percentage native Python class (constructors, conversions, arithmetic)
- PercentageType (constructors, operations)
"""

import pytest

from everybase.type.percentage import Percentage, PercentageType
...
# =============================================================================
# PERCENTAGETYPE TESTS
# =============================================================================


class TestPercentageTypeConstruction:
    """PercentageType construction tests."""

    def test_from_float(self):
        """Create from float."""
        pt = PercentageType.from_float(75.5)
        assert isinstance(pt, PercentageType)
        assert isinstance(pt.source, FuncCallOp)

    def test_from_dec(self):
        """Create from decimal."""
        pt = PercentageType.from_dec(0.755)
        assert isinstance(pt, PercentageType)

    def test_from_bps(self):
        """Create from basis points."""
        pt = PercentageType.from_bps(7550)
        assert isinstance(pt, PercentageType)


class TestPercentageTypeConversions:
    """PercentageType conversion method tests."""
    ...
    def test_to_bps_returns_inttype(self):
        """to_bps() returns IntType."""
        pt = PercentageType.from_float(75.5)
        result = pt.to_bps()
        assert isinstance(result, IntType)

    def test_to_float_returns_floattype(self):
        """to_float() returns FloatType."""
        pt = PercentageType.from_float(75.5)
        result = pt.to_float()
        assert isinstance(result, FloatType)


class TestPercentageTypeApplication:
    """PercentageType application method tests."""

    def test_apply_returns_floattype(self):
        """apply() returns FloatType."""
        pt = PercentageType.from_float(50.0)
        result = pt.apply(200)
        assert isinstance(result, FloatType)

    def test_of_returns_floattype(self):
        """of() returns FloatType."""
        pt = PercentageType.from_float(50.0)
        result = pt.of(200)
        assert isinstance(result, FloatType)
    ...
    def test_add_to_returns_floattype(self):
        ...
        pt = PercentageType.from_float(10.0)
...
class TestPercentageTypeValidation:
    """PercentageType validation method tests."""

    def test_is_valid_returns_booltype(self):
        """is_valid() returns BoolType."""
        pt = PercentageType.from_float(50.0)
        result = pt.is_valid()
        assert isinstance(result, BoolType)

    def test_clamp_returns_percentagetype(self):
        """clamp() returns PercentageType."""
        pt = PercentageType.from_float(150.0)
        result = pt.clamp()
        assert isinstance(result, PercentageType)


class TestPercentageTypeArithmetic:
    """PercentageType arithmetic operation tests."""

    def test_add_returns_percentagetype(self):
        """Addition returns PercentageType."""
        pt = PercentageType.from_float(30.0)
        result = pt + 20.0
        assert isinstance(result, PercentageType)
        assert isinstance(result.source, AddOp)

    def test_sub_returns_percentagetype(self):
        """Subtraction returns PercentageType."""
        pt = PercentageType.from_float(50.0)
        result = pt - 20.0
        assert isinstance(result, PercentageType)
        assert isinstance(result.source, SubOp)

    def test_mul_returns_percentagetype(self):
        """Multiplication returns PercentageType."""
        pt = PercentageType.from_float(25.0)
        result = pt * 2
        assert isinstance(result, PercentageType)
        assert isinstance(result.source, MulOp)

    def test_truediv_returns_percentagetype(self):
        """Division returns PercentageType."""
        pt = PercentageType.from_float(50.0)
        result = pt / 2
        assert isinstance(result, PercentageType)
