...
from every_type import DecimalRef
...
class TestDecimalRefConstructors:
    ...
    def test_from_str(self, ctx):
        """Create from string."""
        result = DecimalRef.from_str("123.456").execute(ctx)
        assert result == Decimal("123.456")

    def test_from_int(self, ctx):
        """Create from integer."""
        result = DecimalRef.from_int(42).execute(ctx)
        assert result == Decimal(42)

    def test_from_float(self, ctx):
        """Create from float (with precision caveat)."""
        result = DecimalRef.from_float(3.14).execute(ctx)
