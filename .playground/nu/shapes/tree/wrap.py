...
def touches_fabric(node: Nu, ref_types: tuple[type, ...]) -> bool:
    ...
    return any(isinstance(ref, ref_types) for ref, _ in tracked_effects(node))
...
def has_write_on_fabric(node: Nu, ref_types: tuple[type, ...]) -> bool:
    ...
    return any(
        eff is Effect.WRITE and isinstance(ref, ref_types) for ref, eff in tracked_effects(node)
    )
    ...
    *,
    ...
    def _walk(node: Nu) -> Nu:
        ...
        if not node._children:
            return node
        new_children = tuple(_walk(c) for c in node._children)
    ...
    *,
    ...
    def _walk(node: Nu) -> Nu:
        ...
        if isinstance(new_node, Flow):
            wrapped = tuple(wrapper(c) for c in new_node._children)
