...
to answer the question, so they bypass NAryScalar's sentinel propagation
...
class IsEmpty(Interaction[bool]):
...
class NotEmpty(Interaction[bool]):
...
class IsInvalid(Interaction[bool]):
...
class NotInvalid(Interaction[bool]):
