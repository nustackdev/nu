...
Composable helpers that return ``Assert`` or ``IfDo`` flow instances.
...
from nu.interactions.command.atomic.assert_ import Assert
from nu.interactions.command.flow.control import IfDo
...
def SkipIfEmpty(  # noqa: N802
    ...
    Returns:
        An ``IfDo`` flow conditioned on ``ref.length() > 0``.
...
def SkipIfMissing(  # noqa: N802
    ...
    Returns:
        An ``IfDo`` flow conditioned on ``ref.exists()``.
...
def SkipIfExists(  # noqa: N802
    ...
    Returns:
        An ``IfDo`` flow conditioned on ``ref.missing()``.
