...
class Shape(metaclass=ShapeMeta):
    ...
    # Market.orders[0].aexecute(ctx) etc.
    # TODO task-079: `aexecute` as a method is gone; use `nu.eval.afirst(ref, ctx)`
    # or `await ref.afetch(ctx)`. Keeping the stub for now since pyright references
    # across the codebase haven't been migrated yet.
    if TYPE_CHECKING:

        async def aexecute(self, ctx: Context) -> object | Sentinel:  # noqa: D102
            ...
