...
Each specialized alias also admits ``AnyForm`` explicitly: the honest
...
Import note: ``AnyForm`` lives in ``nu.forms`` which imports from
...
if TYPE_CHECKING:
    from nu.forms.primitives.any_ import AnyForm
...
type Arg[T] = T | Nu[T] | AnyForm | Sentinel

type IntArg = int | Nu[int] | AnyForm | Sentinel
type FloatArg = float | Nu[float] | AnyForm | Sentinel
type StrArg = str | Nu[str] | AnyForm | Sentinel
type BoolArg = bool | Nu[bool] | AnyForm | Sentinel
type BytesArg = bytes | Nu[bytes] | AnyForm | Sentinel
type NoneArg = None | Nu[None] | AnyForm | Sentinel

type ListArg[V] = list[V] | Nu[list[V]] | AnyForm | Sentinel
type DictArg[K, V] = dict[K, V] | Nu[dict[K, V]] | AnyForm | Sentinel
type SetArg[T] = set[T] | Nu[set[T]] | AnyForm | Sentinel
type FrozenSetArg[T] = frozenset[T] | Nu[frozenset[T]] | AnyForm | Sentinel
type TupleArg[*Ts] = tuple[*Ts] | Nu[tuple[*Ts]] | AnyForm | Sentinel
