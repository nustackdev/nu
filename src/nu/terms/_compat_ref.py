...
from ._compat_nu import LValue
from ._compat_types import Mode, Sentinel, T_co
...
class Ref(LValue[T_co | Sentinel], ABC):
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        # `(own_mode, func_mode)` pair. If `support` is declared, derive legacy
        # modes for runtime dispatch; otherwise enforce the legacy pair.
    ...
    def open(self, ctx: Context) -> Generator[T_co | Sentinel, None, None]:
        ...
        if self.effective_mode is Mode.ASYNC:
            msg = f"{type(self).__name__} has ASYNC in its subtree; cannot run sync."
            raise RuntimeError(msg)
