...
from everylink.attach import AttachError, BaseResourceDescriptor
from everylink.common.descriptor import StorageStrategy, ValidationStrategy

from .coordinator import FleetCoordinator


if TYPE_CHECKING:
    from everylink.resource import Resource
    from everylink.runtime import DependencyManager
...
class FleetDescriptor[ResourceTypeT: Resource](BaseResourceDescriptor[ResourceTypeT]):
    ...
    def __init__(
        ...
        self.specs: tuple[Spec, ...] = specs or ()
        self.alias = alias

    def _validate_type(self, value: object) -> TypeGuard[ResourceTypeT]:
        """Validate that value is a FleetCoordinator (or None)."""
        return value is None or isinstance(value, FleetCoordinator)

    def _get_default(self) -> ResourceTypeT:
        """Default value."""
        raise AttachError("FleetDescriptor has no default value until resolved.")
    ...
    def resolve(
        ...
        for i, spec in enumerate(specs):
            try:
                ...
                raise AttachError(
                    f"Failed to resolve resource at index {i} for AttachFleet '{name}' with spec '{spec}' in '{parent.readable_name}': {e!s}"
                ) from e
...
def AttachFleet(specs: tuple[Spec, ...] | None = None, /, *, alias: str | None = None) -> Any:  # noqa: N802
