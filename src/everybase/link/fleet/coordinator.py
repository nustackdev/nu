...
from everylink.attach import ListCoordinator
...
class FleetCoordinator[ResourceTypeT: Resource](ListCoordinator[ResourceTypeT]):
    ...
    def __init__(self, resources: list[ResourceTypeT]) -> None:
    ...
    def submit(self, method: Callable, *args: object, **kwargs: object) -> Future[Any]:
    ...
    def distribute(
        ...
        # Distribute jobs across workers using round-robin
        for i, (job_args, job_kwargs) in enumerate(zip(args_list, kwargs_list, strict=False)):
    ...
    def _submit_to_resource(
        self, resource: ResourceTypeT, method: Callable, *args: object, **kwargs: object
    ) -> Future[Any]:
        ...
        def execute() -> object:
            try:
                if not inspect.ismethod(method):
                    raise TypeError(
                        f"{method} is not a method, must be a bound method to a everyflow resource"
                    )
    ...
    def __del__(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error during FleetCoordinator cleanup: {e!s}")
