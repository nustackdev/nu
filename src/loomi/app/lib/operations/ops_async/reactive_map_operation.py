...
class ReactiveMapOperation(BaseOperation):
    """Reactively maps an operation to items in a dictionary as they are added.
    ...
    Args:
        operation: Operation to execute for each item
        ...
        class MonitorTransactions(App):
            def define(self) -> Operation:
                return ReactiveMapOperation(
                    operation=TransactionProcessOperation(),
    ...
    async def _process_item(
        ...
        """Process a single item with the operation."""
        ...
        try:
            ...
            # Create context for this operation execution
            operation_context = {
                "map_key": item_key,
            }

            # Execute the operation with the item-specific location
            self.operation.update_context(self.context)  # Pass parent context
            self.operation.update_context(operation_context)  # Add item-specific context

            logger.info(f"Executing operation for item {item_key}")
            await self.operation.execute(app, item_loc)
            logger.info(f"Operation completed successfully for item {item_key}")
            ...
            if self.completion_operation:
                try:
                    ...
                    self.completion_operation.update_context(self.context)  # Pass parent context
    ...
    async def execute(self, app: "AsyncApp", loc: "AsyncStateDictProtocol") -> None:
        ...
        logger.info(f"Starting ReactiveMapOperation for key {self.watch_key}")
        ...
        try:
            ...
            try:
                ...
                logger.info("ReactiveMapOperation was cancelled")
            ...
            if not isinstance(e, asyncio.CancelledError):
                logger.error(f"ReactiveMapOperation failed: {str(e)}", exc_info=True)
                raise OperationError(f"ReactiveMapOperation failed: {str(e)}") from e
            raise
